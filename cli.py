...
class TripPilotCLI:
    ...
    def print_banner(self):
        """打印欢迎横幅"""
        self.console.print("\n[bold cyan]🌏 TripPilot AI 商旅助手[/bold cyan] - 让差旅更简单\n", style="bold")
    ...
    async def initialize_system(self):
        ...
        if not LLM_CONFIG.get("api_key") or not LLM_CONFIG.get("model_name"):
            self.console.print(
                "\n[bold red]LLM 配置缺失。[/bold red]\n"
                "请复制 `.env.example` 为 `.env`，并填写 `TRIPPILOT_LLM_API_KEY` 和 `TRIPPILOT_LLM_MODEL`。\n",
                style="red",
            )
            raise RuntimeError("Missing TRIPPILOT_LLM_API_KEY or TRIPPILOT_LLM_MODEL")
...
def run_health_check_standalone() -> int:
    ...
    import asyncio
    if not LLM_CONFIG.get("api_key") or not LLM_CONFIG.get("model_name"):
        print("FAIL: missing TRIPPILOT_LLM_API_KEY or TRIPPILOT_LLM_MODEL")
        print("Hint: cp .env.example .env and fill in your SiliconFlow credentials.")
        return 1
    init_agentscope()
...
def main():
    ...
    try:
        ...
        if "Missing TRIPPILOT_LLM_API_KEY" in str(e):
            exit(1)
        raise

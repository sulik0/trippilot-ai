...
基于**硅基流动 OpenAI 兼容大模型**和**AgentScope框架**的多智能体旅行规划系统，采用Plan-and-Execute架构，实现智能意图识别、两层记忆系统、RAG知识库、联网搜索和优先级并行调度。

## 快速运行

> AgentScope 1.0.16 要求 Python >= 3.10。当前项目已验证可用 Python 3.13。

```bash
# 1. 创建虚拟环境（如果已有 .venv313 可跳过）
python3.13 -m venv .venv313

# 2. 安装依赖
.venv313/bin/python -m pip install -r requirements.txt

# 3. 配置 SiliconFlow OpenAI 兼容接口
cp .env.example .env
# 编辑 .env，填写 TRIPPILOT_LLM_API_KEY 和 TRIPPILOT_LLM_MODEL
...
### 🧠 两层记忆架构
- **短期记忆**：进程内滑动窗口（默认10轮对话），用于当前会话上下文理解
- **长期记忆**：本地JSON持久化，记录用户偏好、历史行程与聊天摘要
- 智能识别偏好追加/覆盖动作（"我还喜欢如家" vs "我搬家到上海了"）
- 可平滑扩展到 Redis / PostgreSQL 等生产级存储
...
**配置说明**：
- `TRIPPILOT_LLM_API_KEY`: SiliconFlow API Key（必填）
- `TRIPPILOT_LLM_MODEL`: OpenAI 兼容模型名称，默认 `Pro/deepseek-ai/DeepSeek-R1`
- `TRIPPILOT_LLM_TEMPERATURE`: 控制生成随机性（0-1，默认0.7）
- `TRIPPILOT_LLM_MAX_TOKENS`: 最大输出token数（默认8192）
...
- 🤖 **SiliconFlow OpenAI 兼容接口（默认 DeepSeek-R1）** - 大语言模型

### 数据存储
- 🗄️ **本地JSON文件** - 长期记忆持久化（用户偏好、历史行程、聊天记录）
- ⚡ **进程内滑动窗口** - 短期会话记忆
- 🔍 **Milvus Lite** - 本地向量数据库（RAG知识库）
...
### 模型配置
- 必须在 `.env` 中配置 SiliconFlow API Key
- 默认模型为 `Pro/deepseek-ai/DeepSeek-R1`，可按需切换为其他 OpenAI 兼容模型
